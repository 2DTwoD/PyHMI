...
class DActuatorService(DActuatorData):
    ...
    def update(self):
        ...
        self.changed = data[0] & 0x1 > 0
        self.start = data[0] & 0x2 > 0
        self.auto = data[0] & 0x4 > 0
        self.modeling = data[0] & 0x8 > 0
        self.err_reset = data[0] & 0x10 > 0
        self.status = data[0] >> 8
        self.auto_start = data[1] & 0xff
        self.auto_start_mask = data[1] >> 8
        self.auto_stop = data[2] & 0xff
        self.auto_stop_mask = data[2] >> 8
        self.locks = data[3] & 0xff
        self.locks_mask = data[3] >> 8
        self.errors = data[4] & 0xff
